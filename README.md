...
* **[DXVK](https://github.com/doitsujin/dxvk)** by Philip Rebohle, Joshua Ashton, Robin Kertels and Jeffrey Ellison (zlib/libpng) — the `dxbc` shader translator, vendored at [`3rdparty/dxbc`](3rdparty/dxbc). Steam builds of Lossless Scaling ship the shaders as DXBC rather than SPIR-V, so they are translated on import.
* **[Lossless Scaling](https://store.steampowered.com/app/993090/Lossless_Scaling/)** by THS — the shaders themselves. They are read from the user's own installed copy at runtime and none are redistributed here.
* **[LSFG-Android](https://github.com/FrankBarretta/LSFG-Android)** by FrankBarretta (MIT) — the first project to run the lsfg-vk pipeline on Android, and a reference while this port was written. It takes a different route, compositing over a `MediaProjection` capture in a system overlay rather than inside a renderer, so no code is shared with it.
